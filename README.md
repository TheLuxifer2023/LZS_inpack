...
- ✅ **Упаковка обратно** BMFont + DDS → .phyre ✨ **COMPLETE!**
...
- `LZS_inpack.exe -texture font.phyre` - полная экстракция шрифта
- `LZS_inpack.exe -packfont font.fnt texture.dds output.phyre` - упаковка шрифта
...
**Конвертация:**
- `LZS_inpack.exe -topng texture.dds texture.png` - DDS → PNG
...
4. PACK     → LZS_inpack.exe -packfont font.fnt texture.dds modified.phyre
             (создаётся полноценный .phyre файл, размер точно как оригинал!)
             
5. USE      → Замени оригинальный файл в игре
...
**Features:** Extract ✅ | Pack ✅ | Modify ✅ | Convert ✅ | Size Match ✅
